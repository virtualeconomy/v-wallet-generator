...
object WalletGenerator extends App {
  ...
  val WalletSpecification = "1.0"
  val AgentName = "V Wallet Generator"
  val AgentVersion = "0.1.0"
  val AgentString = "V Systems Wallet Specification:" + WalletSpecification + "/" + AgentName + ":" + AgentVersion
  ...
  private def generatePhrase = {
    ...
    var x: Long = 0
    for (i <- 1 to 5) { // 32 * 5 = 160 bits of entropy
      new SecureRandom().nextBytes(r)
      x = (r(3).toLong & 0xff) + ((r(2).toLong & 0xff) << 8) + ((r(1).toLong & 0xff) << 16) + ((r(0).toLong & 0xff) << 24)
      w1 = (x % wordCount).toInt
      w2 = ((((x / wordCount) >> 0) + w1) % wordCount).toInt
      w3 = ((((((x / wordCount) >> 0) / wordCount) >> 0) + w2) % wordCount).toInt
      words += wordList(w1) + " "
      words += wordList(w2) + " "
      words += wordList(w3) + " "
    }
